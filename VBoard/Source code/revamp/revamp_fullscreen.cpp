...
void Draw(Mat edge, Mat canvas) {
	...
	for (int i = 0; i < contours.size(); i++) {
		...
		if (area > 500) {
			...
			//cout << (-(cord.x - 650) + Res.left) << "--" << cord.y << endl;
			expox = (winx / 650.0) + .07;
			expoy = (winy / 540.0) + .25;
			//cout << expoy << endl;
			// << expoy << endl;
			//if (GetCursorPos(&p)) {
			//	cout << "Cursor : " << "(" << p.x << "," << p.y << ")" << endl;
			//	cout << "Object : " << "(" << -(cord.x - 650) * expox << "," << cord.y * expoy << ")" << endl;
			//}
			...
			if (!(cord.y > 450)) {
				for (int i = 0; i < 3; i++) {
					...
					/*
					//forward
					circle(canvas, Point(-(cord.x - 650) + i, cord.y), 2, Scalar(0, 0, 0), FILLED, 1000);
					circle(canvas, Point(-(cord.x - 650), cord.y + i), 2, Scalar(0, 0, 0), FILLED, 1000);
					circle(canvas, Point(-(cord.x - 650) + i, cord.y + i), 2, Scalar(0, 0, 0), FILLED, 1000);
					//backward	
					circle(canvas, Point(-(cord.x - 650) - i, cord.y), 2, Scalar(0, 0, 0), FILLED, 1000);
					circle(canvas, Point(-(cord.x - 650), cord.y - i), 2, Scalar(0, 0, 0), FILLED, 1000);
					circle(canvas, Point(-(cord.x - 650) - i, cord.y - i), 2, Scalar(0, 0, 0), FILLED, 1000);
